...
echo "CREATE SCHEMA $EXASOL_SCHEMA;" | ./usr/opt/EXASuite-7/EXASolution-${EXA_RE_VERSION?}/bin/Console/exaplus -c n11 -u sys -P exasol
...
while [ "$END" == '' ]; do
    sleep 1
	trap "/usr/opt/EXASuite-7/EXAClusterOS-${EXA_RE_VERSION?}/devel/docker/exadt stop && END=1" INT TERM
done
